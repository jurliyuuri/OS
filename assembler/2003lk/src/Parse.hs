...
toInstructions strs = do
  (ils, P {kueList = kl, xokList = xl}) <-
    toI strs `runStateT` P {isCI = False, kueList = [], xokList = []}
...
toI ::
     [String] -> StateT ParserStat (Either Error) [(Maybe Instruction, [Label])]
toI [] = return []
...
parseL [a, b, '@'] = do
  ...
  return $ RPlusNum re 0
parseL [a, b, '+', c, d, '@']
  | isRight (parseRegister [c, d]) =
    let re2 = fromRight (parseRegister [c, d])
     in do re <- parseRegister [a, b]
           return $ RPlusR re re2
parseL (a:b:'+':xs@(_:_))
