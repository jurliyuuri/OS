...
unwrapWith ::
     (Hardware, Program) -> VIO Bool -> (Either Error (Bool, Hardware), Logs)
unwrapWith (initHW, program) =
  runWriter . runExceptT . (`runStateT` initHW) . (`runReaderT` program)
...
finalize = do
  ...
  if a /= initialF5
    then runtimeError $
         "f5 register was not preserved after the call. It should be in " ++
         show initialF5 ++ " but is actually in " ++ show a
    else return False
...
setHigh8bitOfRegister F0 v =
  modify $ \(cpu, m) -> (cpu {f0 = modifyHigh8bit v (f0 cpu)}, m)
setHigh8bitOfRegister F1 v =
  modify $ \(cpu, m) -> (cpu {f1 = modifyHigh8bit v (f1 cpu)}, m)
setHigh8bitOfRegister F2 v =
  modify $ \(cpu, m) -> (cpu {f2 = modifyHigh8bit v (f2 cpu)}, m)
setHigh8bitOfRegister F3 v =
  modify $ \(cpu, m) -> (cpu {f3 = modifyHigh8bit v (f3 cpu)}, m)
setHigh8bitOfRegister F5 v =
  modify $ \(cpu, m) -> (cpu {f5 = modifyHigh8bit v (f5 cpu)}, m)
setHigh8bitOfRegister XX v =
  modify $ \(cpu, m) -> (cpu {xx = modifyHigh8bit v (xx cpu)}, m)
...
setHigh16bitOfRegister F0 v =
  modify $ \(cpu, m) -> (cpu {f0 = modifyHigh16bit v (f0 cpu)}, m)
setHigh16bitOfRegister F1 v =
  modify $ \(cpu, m) -> (cpu {f1 = modifyHigh16bit v (f1 cpu)}, m)
setHigh16bitOfRegister F2 v =
  modify $ \(cpu, m) -> (cpu {f2 = modifyHigh16bit v (f2 cpu)}, m)
setHigh16bitOfRegister F3 v =
  modify $ \(cpu, m) -> (cpu {f3 = modifyHigh16bit v (f3 cpu)}, m)
setHigh16bitOfRegister F5 v =
  modify $ \(cpu, m) -> (cpu {f5 = modifyHigh16bit v (f5 cpu)}, m)
setHigh16bitOfRegister XX v =
  modify $ \(cpu, m) -> (cpu {xx = modifyHigh16bit v (xx cpu)}, m)
...
executeInstruction (Dto r l) =
  templ (\x y -> x `shift` negate (fromIntegral y)) r l
...
executeInstruction (Latsna r ll lh) = do
  ...
  v2 <- getValueFromR (L ll)
  let prod =
        (fromIntegral (fromIntegral v1 :: Int32) :: Word64) *
        (fromIntegral (fromIntegral v2 :: Int32) :: Word64)
  set64bitProd (lh, ll) prod
...
executeInstruction (Krz8i r l) {- load highest 8bit from r; sign-extend; write to l -}
 = do
  val1 <- signExtendFrom8 <$> getHighest8bitFromR r
  setValueToL l val1
executeInstruction (Krz16i r l) {- load highest 16bit from r; sign-extend; write to l -}
 = do
  val1 <- signExtendFrom16 <$> getHighest16bitFromR r
  setValueToL l val1
executeInstruction (Krz8c r l) {- load from r; truncate to 8bit; write to highest 8bit of l -}
 = do
  val1 <- truncateTo8 <$> getValueFromR r
  setValueToHighest8bitOfL l val1
executeInstruction (Krz16c r l) {- load from r; truncate to 16bit; write to highest 16bit of l -}
 = do
...
signExtendFrom8 =
  ...
  (fromIntegral :: Int8 -> Int32) . (fromIntegral :: Word8 -> Int8)

signExtendFrom16 :: Word16 -> Word32
signExtendFrom16 =
  (fromIntegral :: Int32 -> Word32) .
  (fromIntegral :: Int16 -> Int32) . (fromIntegral :: Word16 -> Int16)
  ...
  case readNX program currentNX of
    Nothing
      ...
      | currentNX == debugOutputAddress -> do
        ...
        return ret
      | otherwise ->
        runtimeError $ "nx has an invalid address " ++ show currentNX
